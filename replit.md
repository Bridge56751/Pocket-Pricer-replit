...
- **eBay Sold Search:** `POST /api/ebay-sold-search` retrieves eBay sold item data, providing price statistics and a "Buy Score." AI-powered query cleaning (using Gemini 2.5 Flash via Replit AI Integrations) enhances search accuracy. Uses a two-provider cascade: **SerpAPI (primary, 18s timeout)** → SearchAPI.io (fallback, 12s timeout) when SerpAPI fails. SerpAPI is currently primary because SearchAPI has been failing on common products; the order can be flipped back when SearchAPI's reliability returns. On a strict-zero result from SerpAPI, the auto-broaden retry also runs through SerpAPI. Hard cap of 2 external calls per request. SerpAPI's eBay engine response is normalized into the SearchAPI shape so a single parser handles both providers.

...
- **eBay Sold Search:** `POST /api/ebay-sold-search` retrieves eBay sold item data, providing price statistics and a "Buy Score." AI-powered query cleaning (using Gemini 2.5 Flash via Replit AI Integrations) enhances search accuracy. Uses a **SearchAPI-first waterfall**: SearchAPI.io (12s timeout) is tried first; SerpAPI (18s timeout) is only invoked as a fallback when SearchAPI fails outright (timeout, HTTP error, fetch error) or returns zero usable priced results. SerpAPI is never called when SearchAPI succeeds — best case is 1 external call (~1-2s), worst case is 2 calls (~30s = 12s + 18s). On a strict-zero outcome where at least one provider responded successfully (i.e. real "no results" rather than an outage), the route auto-broadens via a second SearchAPI-first waterfall using a relaxed query (max 4 external calls per request, only on the strict-zero path). `serviceError: true` is only returned when both providers fail outright; one ok-empty + one failure is treated as `noResults: true`. SerpAPI's eBay engine response is normalized into the SearchAPI shape so a single parser handles both providers.

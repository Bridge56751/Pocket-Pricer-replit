...
- **eBay Sold Search:** `POST /api/ebay-sold-search` retrieves eBay sold item data, providing price statistics and a "Buy Score." AI-powered query cleaning (using Gemini 2.5 Flash via Replit AI Integrations) enhances search accuracy. Uses a **parallel-race cascade**: SerpAPI (18s timeout) and SearchAPI.io (12s timeout) are fired simultaneously and the response from whichever provider returns first with usable priced results wins; the loser continues in the background harmlessly. If the faster provider returns zero results, the route waits for the slower one before deciding. This eliminates the cold-query latency cliff that the previous waterfall cascade had (where a slow primary forced a full timeout before falling back). Hard cap of 2 external calls per query. On a strict-zero outcome where at least one provider responded successfully, the route auto-broadens via a second parallel race using a relaxed query (max 4 external calls per request, only on the strict-zero path). `serviceError: true` is only returned when both providers in a race fail outright; one ok-empty + one failure is treated as `noResults: true`. SerpAPI's eBay engine response is normalized into the SearchAPI shape so a single parser handles both providers.

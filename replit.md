...
- **eBay Sold Search:** `POST /api/ebay-sold-search` retrieves eBay sold item data, providing price statistics and a "Buy Score." AI-powered query cleaning (using Gemini 2.5 Flash via Replit AI Integrations) enhances search accuracy. Uses a two-provider cascade: SearchAPI.io (primary, 12s timeout) → SerpAPI (fallback, 18s timeout) when SearchAPI fails. Hard cap of 2 external calls per request. SerpAPI's eBay engine response is normalized into the SearchAPI shape so a single parser handles both providers.
